...
sorted(num_friends_by_id, key = lambda data: data[1], reverse=True)

def friends_of_friends_ids_bad(user):
    # return [foaf['id'] for friend in friends['friends'] for foaf in user['friends']]
...
#print(data_scientists_who_like('theory'))
...
def most_common_interests_with(user):
    return Counter(interested_user_id
                   for interest in interests_in_user_id[user['id']]
                   for interetest_user_id in user_ids_by_interests[interest]
                   if interetest_user_id != user['id'])
